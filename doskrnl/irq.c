...
{
    struct KeIrqRange *range;
    range = MmAllocateZero(sizeof(struct KeIrqRange));
    if(range == NULL) {
        KePanic("Out of memory");
    }
    ...
    irq_handler_t *handler)
{
    ...
    for(i = 0; i < g_irq_table.n_ranges; i++) {
        struct KeIrqRange *range = &g_irq_table.ranges[i];
        irq_t irq;
