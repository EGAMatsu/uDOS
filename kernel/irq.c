...
static irq_handler_t *irq_add_handler_to_line(
    struct irq_line *line, irq_handler_t *handler) {
    line->handlers = krealloc_array(
        line->handlers, line->n_handlers + 1, sizeof(struct irq_line));
...
static irq_t irq_alloc_from_range(
    struct irq_range *range, irq_handler_t *handler) {
    size_t i;
