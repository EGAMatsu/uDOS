...
static struct irq_table g_irq_table = {0};

static struct irq_range *irq_add_range(
    struct irq_range *range)
{
    g_irq_table.ranges = krealloc_array(g_irq_table.ranges,
...
struct irq_range *irq_create_range(
    size_t start,
    size_t n_lines,
...
static irq_handler_t *irq_add_handler_to_line(
...
static irq_t irq_alloc_from_range(
    struct irq_range *range,
    irq_handler_t *handler)
{
    size_t i;
