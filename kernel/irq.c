...
static struct irq_range *irq_add_range(struct irq_range *range) {
    g_irq_table.ranges = krealloc_array(g_irq_table.ranges, g_irq_table.n_ranges + 1, sizeof(struct irq_range));
    if(g_irq_table.ranges == NULL) {
        kpanic("Out of memory");
    }
    memcpy(&g_irq_table.ranges[g_irq_table.n_ranges++], range, sizeof(struct irq_range));
...
static irq_handler_t *irq_add_handler_to_line(struct irq_line *line, irq_handler_t *handler) {
    line->handlers = krealloc_array(line->handlers, line->n_handlers + 1, sizeof(struct irq_line));
...
static irq_t irq_alloc_from_range(struct irq_range *range, irq_handler_t *handler) {
    size_t i;
