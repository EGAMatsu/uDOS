...
static struct jcl_variable *jcl_create_variable(struct jcl_parse_info *info,
                                                const char *key,
                                                const char *value) {
  info->variables = krealloc_array(info->variables, info->n_variables + 1,
                                   sizeof(struct jcl_variable));
...
static struct jcl_dataset *jcl_create_dataset(struct jcl_parse_info *info,
                                              const char *value) {
  struct jcl_dataset *dataset;
  info->datasets = krealloc_array(info->datasets, info->n_datasets + 1,
                                  sizeof(struct jcl_dataset));
...
static const char *jcl_resolve_variable_value(struct jcl_parse_info *info,
                                              const char *var_name) {
  size_t i;
  if (*var_name == '&') {
    ++var_name;
...
static struct jcl_procedure *jcl_create_procedure(struct jcl_parse_info *info,
                                                  const char *name) {
  info->procedures = krealloc_array(info->procedures, info->n_procedures + 1,
                                    sizeof(struct jcl_procedure));
...
static void jcl_add_param_to_procedure(struct jcl_parse_info *info,
  ...
  proc->params = krealloc_array(proc->params, proc->n_params + 1,
                                sizeof(struct jcl_param));
  if (proc->params == NULL) {
    kpanic("Out of memory");
  }
...
static struct jcl_job *jcl_new_job(struct jcl_parse_info *info, user_t account,
                                   const char *owner) {
  struct jcl_job *job;
...
static struct jcl_param jcl_get_positional(struct jcl_parse_info *info,
                                           const char *key,
                                           const char *fall_value) {
  struct jcl_param param;
  char is_multi = 0;
  size_t len;
...
static struct jcl_param jcl_get_optional(struct jcl_parse_info *info) {
  ...
  /* This code will skip until the next parameter - if there is a comma but then
...
static void jcl_parse(struct jcl_parse_info *info) {
  ...
  while (*info->data != '\0') {
    ...
    if (!strcmp(statement, "JOB")) {
      ...
      param = jcl_get_positional(info, "PRGID",
...
static int ucli_cmd_info(int argc, char **argv) {
  ...
  kprintf("MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND "
          "NONINFRINGEMENT.\n");
  kprintf(
...
int ucli_init(void) {
  ...
  kprintf("Copyright (C) 2021 superleaf1995. All rights reserved. Type `info` "
          "for more info.\n");
