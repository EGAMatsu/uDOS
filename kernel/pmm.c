...
struct pmm_region *pmm_create_region(void *base, size_t size) {
    ...
    for (i = 0; i < MAX_PMM_REGIONS; i++) {
        ...
        heap_block->size =
            ((region->size / 1024) + 1) * sizeof(struct pmm_block);
...
static struct pmm_block *pmm_create_block(struct pmm_region *region,
    size_t size, unsigned char flags, struct pmm_block *next) {
...
void *pmm_alloc(size_t size, size_t align) {
    ...
    for (i = 0; i < MAX_PMM_REGIONS; i++) {
        ...
        while (block != NULL) {
            ...
            /* Also check that it is big enough to hold the object (with
             * alignment)*/
            ...
            if (align) {
                left_size = (current_ptr + block->size - size)
                            - ((current_ptr + block->size - size) % align)
                            - current_ptr;
            ...
            if (right_size) {
                new_block = pmm_create_block(
                    region, right_size, PMM_BLOCK_FREE, block->next);
...
void pmm_free(void *ptr) {
    ...
    for (i = 0; i < MAX_PMM_REGIONS; i++) {
        ...
        if (size != region->size) {
            kpanic("Size recollected %zu... but it should be %zu!",
                (size_t)size, (size_t)region->size);
        }
    }
    return;
}