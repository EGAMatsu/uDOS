...
static struct pmm_block *pmm_create_block(struct pmm_region *region, size_t size, unsigned char flags, struct pmm_block *next) {
...
void *pmm_alloc(size_t size, size_t align) {
    ...
    for(i = 0; i < MAX_PMM_REGIONS; i++) {
        ...
        while(block != NULL) {
            ...
            if(left_size) {
                block->next = pmm_create_block(region, block->size - left_size, PMM_BLOCK_FREE, block->next);
            ...
            if(align && current_ptr % align != 0) {
                kpanic("Unaligned alloc of size %zu with address %p", left_size, current_ptr);
            }

            if(right_size) {
                new_block = pmm_create_block(region, right_size, PMM_BLOCK_FREE, block->next);
...
void pmm_free(void *ptr) {
    ...
    for(i = 0; i < MAX_PMM_REGIONS; i++) {
        ...
        if(size != region->size) {
            kpanic("Size recollected %zu... but it should be %zu!", (size_t)size, (size_t)region->size);
        }
    }
    return;
}