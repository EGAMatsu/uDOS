...
static struct user_account g_user_list[MAX_USERS] = {0};
static struct user_group g_group_list[MAX_GROUPS] = {0};
static user_t current_user = 0;

group_t KeCreateUserGroup(
    const char *name)
{
    struct user_group *group = NULL;
    size_t i;
...
struct user_group *KeGetUserGroupById(
    group_t gid)
{
...
struct user_account *KeGetAccountById(
    user_t uid)
{
